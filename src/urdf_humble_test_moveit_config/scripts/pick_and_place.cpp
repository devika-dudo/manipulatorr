...
public:
    ...
    {
        // Initialize basic ROS components first
        ...
        // Use a timer to initialize MoveIt after the node is fully constructed
        init_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(100),
            std::bind(&PickAndPlaceNode::initializeMoveIt, this));
        
        RCLCPP_INFO(this->get_logger(), "Pick and Place Node starting initialization...");
    }
    ...
    void initializeMoveIt()
    {
        ...
        try {
            ...
            // Start the pick and place timer
            pick_place_timer_ = this->create_wall_timer(
                std::chrono::seconds(1),
                std::bind(&PickAndPlaceNode::checkForPickAndPlace, this));
            
            moveit_initialized_ = true;
            RCLCPP_INFO(this->get_logger(), "Pick and Place Node fully initialized");
...
private:
    ...
    // State - Fixed variable names and added missing variables
    geometry_msgs::msg::PointStamped latest_object_pose_;
    geometry_msgs::msg::Pose latest_pick_pose_;
    ...
    void objectPoseCallback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
    {
        // Store the raw point data
        latest_object_pose_ = *msg;
        
        // Create pick pose from the detected point
    ...
    void checkForPickAndPlace()
    {
        if (!moveit_initialized_) {
            return; // Wait for MoveIt to be initialized
        }
    ...
    bool executePickAndPlace()
    {
        ...
        geometry_msgs::msg::Pose pre_pick_pose = latest_pick_pose_;
        ...
        geometry_msgs::msg::Pose pick_pose = latest_pick_pose_;
